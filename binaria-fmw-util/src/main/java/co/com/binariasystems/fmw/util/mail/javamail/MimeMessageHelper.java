...
import co.com.binariasystems.fmw.util.Assert;
...
public class MimeMessageHelper {
	 ...
	 * @see #addInline(String, DataSource)
	 */
	public void addInline(String contentId, URL resource) throws MessagingException {
		Assert.notNull(resource, "Resource must not be null");
		String contentType = null;
		try {
			contentType = getFileTypeMap().getContentType(new File(resource.toURI()).getName());
		} catch (URISyntaxException e) {
			throw new MessagingException("Has ocurred an error, while message was build", e);
		}
	 ...
	 * @see #addInline(String, DataSource)
	 */
	public void addInline(String contentId, URL resource, String contentType)
			throws MessagingException {

		Assert.notNull(resource, "InputStreamSource must not be null");
		DataSource dataSource = createDataSource(resource, contentType, "inline");
		addInline(contentId, dataSource);
	}
	 ...
	 */
	public void addAttachment(String attachmentFilename, URL resource)
			throws MessagingException {

		String contentType = getFileTypeMap().getContentType(attachmentFilename);
		addAttachment(attachmentFilename, resource, contentType);
	}
	...
	public void addAttachment(
			String attachmentFilename, URL resource, String contentType)
			throws MessagingException {

		Assert.notNull(resource, "InputStreamSource must not be null");
		DataSource dataSource = createDataSource(resource, contentType, attachmentFilename);
		addAttachment(attachmentFilename, dataSource);
	}
	 ...
	 */
	protected DataSource createDataSource(final URL inputStreamSource, final String contentType, final String name) {
